...
// Part 2 - Count valid reports, tolerating a single error
{
  ...
  for (const report of reports) {
    ...
    while (queue.length) {
      ...
      const next = index + 1;
      if (cost) {
        if (next - prev <= 2 && next < report.length) queue.push([prev, next]);
        continue;
      }

      if (next < report.length) queue.push([index, next]);
      if (index >= 0) path.push(report[index]);
    }

    if (path.length >= report.length - 1) validCount++;
  }
